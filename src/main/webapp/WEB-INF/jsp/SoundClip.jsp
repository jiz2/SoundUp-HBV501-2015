    ...
    <jsp:attribute name="su_content">
		<c:choose>
			...
			<c:when test="${(action=='Upload' || action=='Play') && not empty soundclip}">
				...
				<br>
				<a href="/soundclip/play/${soundclip.getName()}.${soundclip.getExt()}">Copy this link address to share this sound clip with friends!</a>
			</c:when>
                    ...
                    <c:forEach items="${soundclips}" var="s">
						<c:choose>
							<c:when test="${!s.getIsPrivate() || user.getName().equals(s.getUploader())}">
								<li>
									<a href="/soundclip/play/${s.getName()}.${s.getExt()}">${s.getName()}.${s.getExt()}</a>
									<c:choose>
